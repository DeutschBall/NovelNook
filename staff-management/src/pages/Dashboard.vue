    ...
    <div class="md-layout">
        ...
        <stats-card data-background-color="blue">
          <template slot="header">
            <span
              ...
              style="width: 1em; height: 1em; font-size: 48px"
            >
              groups
            </span>
          </template>
          ...
          <template slot="footer">
            ...
            <div class="md-layout-item md-medium-size-50 md-xsmall-size-100 md-size-25">
              <stats-card data-background-color="red">
                ...
                <template slot="footer">
                  <md-button class="md-raised md-success" @click="ConfirmFine">
                   Confirm
                  </md-button>
                </template>
              </stats-card>
      ...
      <div class="md-layout-item md-medium-size-50 md-xsmall-size-100 md-size-25">
        <stats-card data-background-color="orange">
          ...
          <template slot="content">
            <p class="category">Max Borrowing</p>
            <md-input
              v-model="maxBook"
              type="text"
          ></md-input>
          ...
          <template slot="footer">
            <md-button class="md-raised md-success" @click="ConfirmBook">
              Confirm
            </md-button>
          </template>
        </stats-card>
          ...
          <el-main>
              ...
              <div class="search_input">
                <el-form :inline="true" :model="searchStaff.username">
                  ...
                  <el-form-item>
                    <el-button
                      type="primary"
                      @click="selectByuserName(searchStaff.username)"
                      >Search</el-button
                    >
            ...
            <el-dialog title="Add New Staff" :visible.sync="dialogAddVisible">
              ...
              <div slot="footer" class="dialog-footer">
                ...
                <el-button
                  type="primary"
                  @click="handleSetAdd(scope.$index, scope.row)"
                  >Save</el-button
                >
              </div>
            </el-dialog>
...
<!--                      :src="require('@/assets/' + scope.row.avatar)">-->
...
<!--                          @click="handleSet(scope.$index, scope.row) "-->
                    ...
                    <el-button
                      type="danger"
                      size="small"
                      @click="bandDeleteName(scope.row)"
                      >Delete</el-button
                    >
                    <el-dialog
                      title="Prompt"
                    ...
                    >
                      ...
                      <span slot="footer" class="dialog-footer">
                        <el-button @click="dialogVisible = false"
                          >Quit</el-button
                        >
                        <el-button type="primary" @click="deleteByuserName()"
                          >Confirm</el-button
                        >
                      </span>
                    </el-dialog>
...
export default {
  ...
  mounted() {
    ...
    //获得罚款和最大借书数的数据
    axios.get("http://localhost:8080/admin/fine").then(function (resp) {
      if (resp.data.code === 1) {
        that.fine = resp.data.data;
        // console.log(resp);
      } else {
        alert(resp.data.msg);
      }
    });
    axios.get("http://localhost:8080/admin/bookLimit").then(function (resp) {
      if (resp.data.code === 1) {
        that.maxBook = resp.data.data;
        // console.log(resp);
      } else {
        alert(resp.data.msg);
      }
  ...
  methods: {
    selectByuserName(userName) {
      let that = this;
      axios
        .get("http://localhost:8080/admin/staff/" + userName)
        .then(function (resp) {
          if (resp.data.code === 1) {
            that.tableData = resp.data.data;
          } else {
    ...
    deleteByuserName() {
      ...
      axios
        .delete("http://localhost:8080/admin/staff/" + that.deleteName)
        .then(function (resp) {
          if (resp.data.code === 1) {
            axios
              .get("http://localhost:8080/admin/staff")
    ...
    ConfirmFine(){
      ...
      axios
          .put("http://localhost:8080/admin/updateMoney",param)
          .then(function (resp) {
            if (resp.data.code === 1) {
             alert("Ok");
            } else {
    ...
    ConfirmBook(){
      let that = this;
      const param={
        limitnum:that.maxBook,
      }
      axios
          .put("http://localhost:8080/admin/updateLimitNumber",param)
          .then(function (resp) {
            if (resp.data.code === 1) {
              alert("Ok");
            } else {
    ...
    //   };
    //   axios
    //     .put("http://localhost:8080/admin/staff", params)
    //     .then(function (resp) {
    //       if (resp.data.code === 1) {
